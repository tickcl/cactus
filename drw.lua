...
SetProperty = function(fvs, index, value)
    if fvs[index] ~= nil then
        ...
        error("Cannot set an invalid property: " .. tostring(index))
    end
end,
GetProperty = function(fvs, index)
    if fvs[index] ~= nil then
        return fvs[index]
    else
        error("Cannot get an invalid property: " .. tostring(index))
        return nil
    end
end,
SetParent = function(fvs, parent)
    fvs.Parent = parent
end
}, {
...
return setmetatable({}, {
    __newindex = function(_, index, value)
        if lineObj[index] == nil then 
            error("Invalid property: " .. tostring(index))
            return 
        end
...
return setmetatable({Parent = drawingUI}, {
    __newindex = function(_, index, value)
        if frameObj[index] == nil then
            error("Invalid drawing lib property: " .. tostring(index))
            return
        end
...
return setmetatable({Parent = coreGui}, {
    __newindex = function(_, index, value)
        if screenGuiObj[index] == nil then
            error("Invalid drawing library property: " .. tostring(index))
            return
        end
    ...
    __newindex = function(_, index, value)
        ...
        elseif index == "MouseButton1Click" then
            if typeof(value) == "function" then
                ...
                error("Invalid value for MouseButton1Click: expected function, got " .. typeof(value))
            end
        end
...
new = newcclosure(function(drawingType)
    ...
    elseif drawingType == "TextBox" then
        ...
        error("Invalid drawing lib type: " .. tostring(drawingType))
    end
end)
}
